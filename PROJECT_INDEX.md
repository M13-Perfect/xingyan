...
Last verified: 2026-07-01 after restart-xingyan.bat LAN-IP auto-detect and VPS user/survey data import into local `xyyx_db`. See `CURRENT_TASKS.md` "2026-07-01 session log" for full detail.

Auth hardening round 2, 2026-07-02 (after the access-control repair below): (1) **Logout now actually revokes the token.** Casdoor issues stateless JWTs, so Casdoor's own logout does NOT stop an issued token validating at our resource server — added `TokenDenylistService` (in-memory session-fingerprint→expiry denylist, TTL = token remaining life; single-JVM, needs Redis for multi-instance) + `TokenDenylistFilter` (registered after `BearerTokenAuthenticationFilter`; returns 401 `TOKEN_REVOKED` when the current token's `CurrentUserService.loginSessionId` fingerprint is denylisted) + `POST /api/auth/logout` (denylists the current session; also expires a future `XYYX_AT` cookie defensively). Frontend `logout()` is now async and `await`s `POST /api/auth/logout` BEFORE `clearAuthStorage()` (otherwise the async axios interceptor would read an already-cleared sessionStorage and send no token), then still calls Casdoor logout. (2) **CORS locked down.** `CorsConfig` dropped `allowedOriginPatterns("*")+allowCredentials(true)` (latent footgun, becomes critical with cookies) for an explicit `xyyx.cors.allowed-origins` allowlist (default `http://localhost:5173,http://127.0.0.1:5173`) scoped to `/api/**`; removed `@CrossOrigin` from SurveyController/NoticeController so they can't re-widen it. Prod/dev are same-origin (reverse proxy / Vite proxy) so CORS is only a "direct :8080" safety net. (3) **Login-password verdict.** Research (HIGH confidence, Casdoor docs+source) confirms `/api/login` is plaintext by default — no RSA; the only option is an optional org-level symmetric "Password Obfuscator" with a client-visible shared key (obfuscation, not encryption). The real protection is TLS, NOT "security through obscurity". No code change to login crypto; added a clarifying comment in `App.vue handleLogin`; real hardening = enforce HTTPS/HSTS + rate-limit + reverse proxy must not log request bodies. Tests: `mvn -o test` 61 green (+`TokenDenylistServiceTests`), `npm test` 31 green.

Auth hardening round 3 (HttpOnly cookie migration, Pattern B relay), 2026-07-02: user approved "implement + I smoke-test". Access token no longer lives in JS/sessionStorage — it is relayed to the backend and stored in an HttpOnly cookie. Backend: `CookieBearerTokenResolver` (reads `XYYX_AT` cookie, falls back to Authorization header during migration) wired into `oauth2ResourceServer`; `POST /api/auth/session` (permitAll) validates the relayed token via the resource-server `JwtDecoder` (JWKS) + `requireUser`, then sets `XYYX_AT` (HttpOnly, Secure, SameSite=Lax, Path=/api, Max-Age=token exp); `GET /api/me/account` returns displayName/avatar via new `CasdoorAdminService.getAccount` (admin Basic auth, best-effort) so the SPA no longer needs a JS-readable token to hit Casdoor get-account. Frontend `App.vue`: deleted `ACCESS_TOKEN_KEY` + the Authorization interceptor (now sets `withCredentials` for /api); `completeCodeLogin` POSTs the token to `/api/auth/session` instead of `sessionStorage.setItem`; `onMounted` restores session by probing `GET /api/me` (no JS token to check); `fetchMyCasdoorAccount` → `/api/me/account`; `clearAuthStorage` no longer touches sessionStorage. CSRF defense = SameSite=Lax cookie + same-origin (Spring CSRF tokens intentionally NOT added — deliberate simplicity, upgrade path noted). Tests: `mvn -o test` 64 green (+`AuthSessionControllerTests`: valid token→HttpOnly cookie, invalid→401 no cookie, missing→400), `npm run build` OK, `npm test` 31 green. NOT VERIFIED HERE (needs a live smoke test before deploy): the real browser login round-trip (Casdoor redirect → /auth/callback → token exchange → POST /api/auth/session sets cookie → subsequent /api calls carry it → logout revokes). Secure cookie works on http://localhost (secure context) and prod HTTPS but NOT on plain-http LAN IP. Header fallback stays on the backend so a rollback = revert only the frontend commit. An adversarial security review (5-dimension workflow + per-finding refutation) was run: 3 findings, 0 confirmed. Two were correctly refuted (logout cookie Path is consistent /api; no-Domain is correct scoping). The third (revocation keyed on session-stable sid could over-revoke a fresh re-login) was refuted only on the unverified assumption that Casdoor emits no stable `sid`; since OIDC `sid` is designed to be session-stable and our Casdoor grounding was inconclusive, it was fixed defensively: added `CurrentUserService.tokenRevocationKey` (per-token: jti else token fingerprint, never sid/sessionId) and switched `TokenDenylistFilter` + `AuthController.logout` to it, so logout revokes exactly the one token and a fresh login is never blocked. `loginSessionId` (phone-reveal, wants session-stability) left unchanged. Re-verified: `mvn -o test` 64 green.

Access-control repair, 2026-07-02: fixed three broken-access-control endpoints in `SurveyController` that had NO `@AuthenticationPrincipal`/authz/row-scoping — `PUT /api/surveys/{id}/process`, `DELETE /api/surveys/{id}`, `PUT /api/surveys/{id}/share`. Any authenticated staff could process/delete/re-share ANY survey by arbitrary id; the share hole was the worst — a staff user could set a foreign private record to `PUBLIC` or add themselves to `shared_users`, then read/reveal its phone (privilege escalation → PII exfiltration). Now: `process` = `requireUser` + admin/staff tenant/STAFF_SCOPE via new `updateAdminStatus`/`updateStaffStatus` (403 `SURVEY_ACCESS_DENIED` on 0 rows); `delete` = `requireAdmin` + tenant-scoped `deleteByIdTenant` (404 on 0 rows); `share` = `requireAdmin` + `ALLOWED_VISIBILITY` whitelist + tenant-scoped `updateVisibilityTenant`. Old unscoped mapper methods `updateStatus`/`deleteById`/`updateVisibility` deleted. Regression: `SurveyControllerAccessControlTests`; `mvn -o test` 60 green. Frontend same session: removed the unused `refresh_token` sessionStorage write in `App.vue` (never read; pure XSS-theft surface); `npm test` 31 + `reviewEdgeCases.test.js` 7 green. Audited `NoticeController`/`AuthController` — already properly `requireUser`/`requireAdmin` + username-scoped, no change. NOT fixed this pass (design tradeoffs needing broader work): access token still in sessionStorage, no server-side token revocation on logout, login password sent plaintext to Casdoor over TLS.

`restart-xingyan.bat` update, 2026-07-01: LAN IP is auto-detected each run via `Get-NetIPConfiguration` (real physical/Wi-Fi NIC only, VPN/tunnel adapters excluded via `NetAdapter.Virtual`), no longer a hardcoded fallback IP. An explicit IP argument still overrides.

Local `xyyx_db` data update, 2026-07-01: imported a fresh VPS business-DB export's `user` (8 real staff, preserving their VPS ids) and `survey` (275 rows, replacing 53 local synthetic phone-privacy test rows that were backed up and discarded) tables, backfilled `owner_user_id`/`created_by_user_id`, and ran `phone-privacy-backfill` (100% coverage, 0 invalid). New one-shot tool `xyyx/src/main/java/org/example/xyyx/task/StaffCasdoorBackfillTask.java` creates Casdoor accounts for existing local users and syncs the local BCrypt hash. 11 survey rows have `owner='lilimin'` with no matching user account anywhere (local or VPS) — confirmed by user as a departed employee, so `owner_user_id` staying NULL (admin-only visibility) is intentional, not a gap.

"开通新员工" nickname field, 2026-07-01: `POST /api/users` now accepts an optional `nickname`, trimmed and capped at 50 chars, resolved to a `displayName` (falls back to `username` if blank) that's passed into `CasdoorAdminService.createUser(username, plainPassword, displayName)` — the signature grew a third required param, so `StaffCasdoorBackfillTask` (the only other caller) passes `username` again to keep its old behavior. Frontend `App.vue` create-staff card has a matching "设置昵称" input, and the username field's placeholder no longer wrongly claims Chinese characters are supported (Casdoor's `/api/add-user` only accepts alnum/underscore/hyphen). Deliberately did NOT add a separate username-duplicate-check API — the existing `POST /api/users` already returns "账号已存在" and is already admin-gated, so a dedicated check endpoint would only add attack surface for no benefit at this low-frequency admin action.
...
Personal center + account cleanup, 2026-07-01: removed dead `POST /api/login` from `SurveyController` (frontend only ever called Casdoor's own `/api/login`, never this backend endpoint) and its now-unreachable `SecurityConfig` permitAll entry. Found and fixed a real bug: admin password reset (`PUT /api/users/{id}/password`) only wrote the local `user.password` mirror and never called Casdoor, so a reset admin performed had zero effect on the actual login password; it now calls `CasdoorAdminService.updateUserPassword` inside the same `@Transactional` boundary as the local write. Added self-service `PUT /api/me/password` (verifies current password against the local BCrypt hash, then updates local + Casdoor) and `PUT /api/me/profile` (writes only `displayName` to Casdoor via `casdoorAdminService.updateUserDisplayName`; nickname is intentionally NOT a local `user` column — Casdoor's `displayName`/`avatar` are the single source of truth, read by the frontend directly via `GET {casdoorBaseUrl}/api/get-account` with the existing Casdoor access token). Added a new frontend "个人中心" modal (avatar-initial button replaces the old plain-text username/role in the top-right corner) covering nickname edit and self-service password change; avatar image upload is intentionally deferred (see Known Risks). The `db/migrations/V20260701_001_redesign_user.sql` / `xyyx_db.sql` `user` table redesign no longer includes `nickname`/`avatar_url` columns for the same reason. Verified with `mvn test` (incl. new structural tests in `SurveyControllerUserManagementTests`), frontend `npm test` (20/20), and `npm run build`; no live Casdoor/MySQL run was performed, so the actual login/nickname/password round-trip against a real Casdoor instance is still unverified.

"开通新员工" root cause, 2026-07-01: the reported staff-creation failure was a browser security-context issue, not a backend bug — `window.crypto.subtle` (needed by `encryptPassword()` for the RSA-OAEP staff-password flow) is disabled by browsers on non-localhost origins served over plain HTTP, and the user was on `http://172.18.0.1:5173` (a `singbox_tun` VPN/proxy tunnel adapter IP, not a real LAN address). Confirmed fixed by using `http://localhost:5173` / `http://127.0.0.1:5173` instead. `getApiErrorMessage` in `App.vue` now surfaces `error.message` for non-network JS errors instead of a content-free fallback string, and `encryptPassword`'s error text explains the secure-context requirement — both kept as permanent improvements.

Dev HTTPS explored and reverted, 2026-07-01: briefly made local/LAN dev serve over HTTPS (`@vitejs/plugin-basic-ssl` in `vite.config.js`, `restart-xingyan.bat` FRONTEND_URL) to let LAN-IP access work with the RSA password screens. This surfaced a deeper issue — Casdoor's own OIDC discovery document always returns `http://` absolute endpoint URLs (confirmed via `curl http://<host>:8000/.well-known/openid-configuration`; it echoes the request host but never the scheme, since Casdoor has no TLS), and the frontend calls `token_endpoint` from that document directly, so a dev-only proxy in front of Casdoor would not have been sufficient — the real fix would have required giving Casdoor its own (self-signed) HTTPS listener via Beego's native `EnableHTTPS`/`HTTPSCertFile`/`HTTPSKeyFile` support, plus keeping the backend's `SPRING_SECURITY_OAUTH2_RESOURCESERVER_JWT_ISSUER_URI`/`JWK_SET_URI` in sync. Decided this was unneeded complexity — LAN/phone testing of login and phone-privacy features already works over plain HTTP (that flow has a WebCrypto-unavailable SHA-256 fallback); only the RSA password screens (staff creation, password change) need a secure context, and there was no stated need to run those specifically from a non-localhost device. Fully reverted: `@vitejs/plugin-basic-ssl` removed, `vite.config.js` and `restart-xingyan.bat` back to plain HTTP. `restart-xingyan.bat`'s default `LAN_IP` fallback was also corrected from the stale `192.168.1.29` to the machine's actual current address `192.168.147.247` (confirmed via `ipconfig`). Standing constraint: staff creation / self-service password change only work from `http://localhost:5173` or `http://127.0.0.1:5173`.

Production is unaffected by any of the above either way — `sso.xujie.xin`'s Nginx already terminates real Let's Encrypt TLS in front of Casdoor's plain-HTTP origin (`proxy_pass http://127.0.0.1:8000`), which is the same shape the reverted local experiment was trying to reproduce; nothing here touches deployment.
...
- Nicknames only exist in Casdoor (`displayName`), never a local column. `CasdoorAdminService.listDisplayNames()` (added 2026-07-02) batch-fetches `/api/get-users?owner=<org>` and is best-effort (empty map on failure). `GET /api/surveys` uses it to add `ownerNickname` per row, admin requests only. `GET /api/users` now also returns per-employee `pendingCount`/`overdueCount` (owner-scoped, not `STAFF_SCOPE`) for the 员工管理 workload column.
...
- Staff management with create/delete/password update; each row's rightmost column shows that employee's live (未处理N，已逾期N) workload, owner-scoped only (2026-07-02).
- Admin-only workbench view shows the recording account's Casdoor nickname under each row's status tag (2026-07-02).
